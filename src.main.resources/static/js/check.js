...
function ShowMes(str,mes,type){
	...
	{	
	  ...
	  {
		  	o=document.getElementById(str+"span");
	//		alert(str+"span");
			if(o==null)
				return;
		 	o.className=type;
...
function init_check(form,checkfunc)
{
	var formobj =document[form];
	 for (var i=0;i<formobj.length;i++) 
	 { 
		formobj.elements[i].onblur=function(e)
		{
				e=window.event||e;
				var eSrc=e.srcElement||e.target;
				if(checkfunc(this.name)==true)
					ShowMes(this.name ,"正确","right_check");
		}
	 } 
}
...
function checkempty(str,errorstr)
{
...
function checkroutename(obj,count,errorstr)
{
	for(var i = 1; i < count; i++) 
	{
		if(obj != ('name'+i))
		{
		    ...
		    if(document.getElementById(obj).value == document.getElementById(('name'+i)).value
			&& document.getElementById(obj).value != "")
			{
		//	document.getElementById(obj).focus();
				ShowMes(obj,errorstr,"wrong_check");
				return false;
			}
		}
	}
...
function checkipex(str,errorstr)
{
	if(checkIP(document.getElementById(str).value)==false)
	{
		ShowMes(document.getElementById(str),errorstr,"wrong_check");
		return -1;
	}	
	return 0;
}

function checkobjip(str,errorstr)
{
	if($(str)==null||isEmpty($F(str))==true)
		return true;
	if(checkIP(document.getElementById(str).value)==false)
	{
...
function checkipornet(str,errorstr)
{
	if($(str)==null||isEmpty($F(str))==true)
		return 0;
	var ip_array = $F(str).split("/");
	if (ip_array.length != 2) 
	{
  		return checkipex(str,errorstr);
  	}else
	{
		if(checkIPNet($F(str))==false)
		{
			ShowMes(document.getElementById(str),errorstr,"wrong_check");
			return -1;
		}		
...
function checkobjipornet(str,errorstr)
{
	if($(str)==null||isEmpty($F(str))==true)
		return true;
	var ip_array = $F(str).split("/");
...
function checkobjnum(str,errorstr)
{
	if($(str)==null||isEmpty($F(str))==true)
		return false;
	if(checkNumber(document.getElementById(str).value)==false)
	{
		//document.getElementById(str).focus();
		ShowMes(str,errorstr,"wrong_check");
				return false;
...
function checkobjtime(str,errorstr)
{
	if($(str)==null||isEmpty($F(str))==true)
		return false;

	if(checkTime(document.getElementById(str).value)==false)
	{
		//document.getElementById(str).focus();
		ShowMes(str,errorstr,"wrong_check");
				return false;
...
function checkobjport(str,errorstr)
{
	if($(str)==null||isEmpty($F(str))==true)
		return true;
...
function checkname(str,errorstr)
{
	if($(str)==null||isEmpty($F(str))==true)
		return true;

	if(checkName_ex(document.getElementById(str).value)==false)
	{
		//document.getElementById(str).focus();
		ShowMes(str,errorstr,"wrong_check");
			return false;
...
function checkmac(str,errorstr)
{
	var reg= /^([0-9a-fA-F]{2}:{1}){5}[0-9a-fA-F]{2}$/g;
...
function checkobjmac(str,errorstr)
{
	var reg= /^([0-9a-fA-F]{2}:{1}){5}[0-9a-fA-F]{2}$/g;
	if(!document.getElementById(str).value.match(reg))
	{
...
function checkconfirm(str1,str2,errorstr)
{
